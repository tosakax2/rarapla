...
class IcyWatcher(QThread):
    ...
    def __init__(self, url: str, user_agent: str | None=None, parent: QObject | None=None) -> None:
            ...
            while self._running:
                try:
                    async with self._session.get(self._url, headers=headers) as resp:
                        self._resp = resp
                        metaint_str = resp.headers.get('icy-metaint') or resp.headers.get('Icy-MetaInt')
                        self._base_meta = self._extract_headers(resp.headers)
                        self.metaUpdated.emit('', dict(self._base_meta))
                        if not metaint_str:
                            self.notSupported.emit('icy-metaint header is missing (no ICY metadata).')
                            return
                        try:
                            ...
                            self.notSupported.emit(f'invalid icy-metaint: {metaint_str!r}')
                            return
                        reader = resp.content
                        while self._running:
                            try:
                                await asyncio.wait_for(reader.readexactly(metaint), timeout=2.0)
                                length_byte = await asyncio.wait_for(reader.readexactly(1), timeout=2.0)
                            except (asyncio.TimeoutError, asyncio.IncompleteReadError):
                                continue
                            block_len = length_byte[0] * 16
                            if block_len:
                                try:
                                    block = await asyncio.wait_for(reader.readexactly(block_len), timeout=2.0)
                                except (asyncio.TimeoutError, asyncio.IncompleteReadError):
                                    continue
