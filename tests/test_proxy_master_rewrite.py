...
def test_handle_master_rewrite(monkeypatch: pytest.MonkeyPatch) -> None:
    m3u8 = "\n".join(
        [
            ...
            "https://cdn.radiko.example/live/FMT/chunklist_b256000.m3u8",
            "",
        ]
    )
    ...
    async def _fake_ensure(station: str) -> ResolvedStream:
        return ResolvedStream(
            station_id=station,
            m3u8_url="https://cdn.radiko.example/live/FMT/master.m3u8",
        )
    ...
    out = resp.text
    exp1 = "/seg.m3u8?" + urlencode(
        {
            "u": "https://cdn.radiko.example/live/FMT/chunklist_b128000.m3u8",
            "station": "FMT",
        }
    )
    exp2 = "/seg.m3u8?" + urlencode(
        {
            "u": "https://cdn.radiko.example/live/FMT/chunklist_b256000.m3u8",
            "station": "FMT",
        }
    )
