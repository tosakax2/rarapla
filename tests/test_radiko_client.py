...
def test_fetch_now_programs_selects_current(
    station_list_xml: str, now_xml_current_hit: str, patch_radiko_client_datetime: bool
) -> None:
    table = {
        "https://radiko.jp/v2/station/list/JP12.xml": __build_resp(station_list_xml),
        "http://radiko.jp/v3/program/now/JP12.xml": __build_resp(now_xml_current_hit),
    }
...
def test_fetch_program_detail_date_preferred(
    date_xml_has_now: str, patch_radiko_client_datetime: bool
) -> None:
    station = "FMT"
    ymd = "20250102"
    table = {
        f"https://radiko.jp/v3/program/station/date/{ymd}/{station}.xml": __build_resp(
            date_xml_has_now
        )
    }
    cli = RadikoClient(session=__build_session(table))
...
def test_fetch_program_detail_weekly_fallback(
    weekly_xml_fallback: str, patch_radiko_client_datetime: bool
) -> None:
    station = "FMT"
    ymd = "20250102"
    table = {
        ...
        f"https://radiko.jp/v3/program/station/weekly/{station}.xml": __build_resp(
            weekly_xml_fallback
        ),
    }
    cli = RadikoClient(session=__build_session(table))
